...
const About = () => {
  ...
  return (
    <section id="about" className="relative bg-white py-12 md:py-24 overflow-hidden">
      ...
      <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 w-96 h-96 bg-[#f2ca1c]/10 rounded-full blur-3xl opacity-50" />
      <div className="absolute bottom-0 left-0 translate-y-1/2 -translate-x-1/2 w-96 h-96 bg-[#31468e]/10 rounded-full blur-3xl opacity-50" />
      ...
      <div className="w-full max-w-7xl mx-auto px-[15px] md:px-6 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {aboutValues.map((value, index) => (
                <div key={index} className="flex gap-4 p-4 rounded-xl hover:bg-gray-50 transition-colors">
                  <div className="w-1.5 h-12 bg-[#31468e] rounded-full flex-shrink-0" />
                  <div>
          ...
          >
            <div className="flex flex-col md:flex-row items-center gap-6 md:gap-12">
              <div className="md:w-1/3 px-[15px] md:px-0">
                <div className="bg-[#31468e] w-16 h-16 rounded-2xl flex items-center justify-center mb-6 shadow-lg shadow-[#31468e]/30">
                  <Gamepad2 className="text-[#f2ca1c] w-8 h-8" />
                </div>
                <h3 className="text-3xl font-black text-slate-900 mb-4 tracking-tight">Interactive <span className="text-[#31468e]">Lounge</span></h3>
        ...
        <motion.div
          ...
          className="w-full grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 mt-16 md:mt-24 bg-[#31468e] rounded-[2rem] md:rounded-3xl p-6 md:p-10 shadow-2xl relative overflow-hidden"
        >
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(242,202,28,0.1),transparent_50%)]" />
          {aboutStats.map((stat, index) => (
            <div key={index} className="text-center group relative z-10">
              <div className="flex justify-center mb-4">
                <stat.icon className="w-10 h-10 text-[#f2ca1c] group-hover:scale-110 transition-transform" />
              </div>
              <div className="text-4xl font-bold text-white mb-2">{stat.value}</div>
              <div className="text-white/60 text-sm font-medium uppercase tracking-wide">{stat.label}</div>
            </div>
          ))}
        </motion.div>
      </div>
